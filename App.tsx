...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Load usage and subscription status
  ...
  // Fix: The default React.CSSProperties type does not include CSS custom properties (e.g., '--primary-500').
  // Using 'any' to bypass the type check for this specific case, as the object is a valid style object for React.
  const customThemeStyles: any = useMemo(() => {
    if (theme.name !== 'custom' || !theme.primaryColor || !theme.accentColor) {
      return {};
    }
    return {
      '--primary-50': adjustColor(theme.primaryColor, 0.9),
      '--primary-100': adjustColor(theme.primaryColor, 0.8),
      '--primary-200': adjustColor(theme.primaryColor, 0.6),
      '--primary-300': adjustColor(theme.primaryColor, 0.4),
      '--primary-400': adjustColor(theme.primaryColor, 0.2),
      '--primary-500': theme.primaryColor,
      '--primary-600': adjustColor(theme.primaryColor, -0.1),
      '--primary-700': adjustColor(theme.primaryColor, -0.2),
      '--primary-800': adjustColor(theme.primaryColor, -0.3),
      '--primary-900': adjustColor(theme.primaryColor, -0.4),
      '--accent-300': adjustColor(theme.accentColor, 0.4),
      '--accent-500': theme.accentColor,
      '--accent-600': adjustColor(theme.accentColor, -0.1),
      '--accent-700': adjustColor(theme.accentColor, -0.2),
      '--accent-900': adjustColor(theme.accentColor, -0.4),
      '--accent-contrast': getContrastingTextColor(theme.accentColor),
    };
  }, [theme]);
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (!calendarText.trim()) {
      setError('Please paste your calendar schedule first.');
      return;
    }
    ...
    try {
      ...
      console.error(err);
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred while fetching your recommendation.';
  ...
  const handlePositiveFeedback = () => {
    console.log("Feedback Received: Positive (Helpful)");
    setFeedbackState('positive');
  };

  const handleNegativeFeedback = () => {
    console.log("Feedback Initiated: Negative (Not Helpful)");
    setFeedbackState('negative-prompt');
  };
  
  const handleFeedbackSubmit = (feedback: string) => {
    console.log("Feedback Submitted (Text):", feedback);
  ...
  const handleClearSavedData = useCallback(() => {
    if (window.confirm("Are you sure you want to clear your saved session? This action cannot be undone.")) {
  ...
  const handleShare = () => {
    const shareText = "Just found my perfect yoga flow for the day with ZenFlow AI! It analyzes your calendar to recommend a personalized practice. 🧘‍♀️✨ #ZenFlowAI #Yoga #Wellness";
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}`;
    window.open(twitterUrl, '_blank');
    
    setUsageCount(0); // Reset for 3 more uses
    localStorage.setItem(USAGE_COUNT_KEY, '0');
    setShowPaywall(false);
  };

  const handleSubscribe = () => {
    console.log("Simulating subscription purchase for $75/year.");
    alert("Thank you for subscribing! You now have unlimited access to ZenFlow AI.");
    ...
    >
      ...
      <div className="w-full max-w-4xl mx-auto">
        ...
        <header className="text-center mb-8">
            <h1 className="text-4xl sm:text-5xl font-bold text-[var(--primary-800)] tracking-tight">
              ZenFlow AI
            </h1>
            <p className="text-lg text-[var(--primary-600)] mt-2">
              Your personal yoga recommender based on your day's schedule.
            </p>
        </header>
        ...
        <main className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 space-y-6">
          ...
          {calendarText.trim().length > 0 && !isLoading && !recommendation && (
            <BusynessVisualizer score={busynessScore} />
          )}
             ...
             <div className="text-center text-gray-500 p-8 border-2 border-dashed border-gray-200 rounded-lg">
              <p className="text-lg">Your personalized yoga recommendation will appear here.</p>
               {calendarText.trim().length === 0 && (
                <p className="mt-2 text-sm">Paste your schedule above to see an estimate of your day's busyness and find your flow.</p>
              )}
            </div>
          )}
        </main>
